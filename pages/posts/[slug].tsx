...
import {
  API_ENDPOINT_DETAIL_ARTICLE,
  API_ENDPOINT_ARTICLE,
  API_ENDPOINT_COMMENT,
...
} from "../../components/blog/post-comment";
    ...
    return (
      <>
        <div className="absolute inset-x-0 bottom-0 flex flex-col items-center justify-end pb-8 mx-2 lg:max-w-2xl lg:mx-auto bg-gradient-to-b from-transparent via-white to-white">
          <h4 className="text-3xl font-medium leading-9 text-primary mt-52 ">
            Jadilah Bagian dari Kami!
          </h4>
          <p className="text-lg leading-loose text-center text-gray-700 ">
            Dapatkan akses tanpa batas ke seluruh artikel kami dengan
            berlangganan comika.media
          </p>
          <button
            type="button"
            className="text-xl"
...
export default function DetailOfPost({
  ...
  const { data: postClient, mutate: mutatePost } = useSWR(
    `${API_ENDPOINT_DETAIL_ARTICLE}/${post.slug}`,
    client.get,
    { fallbackData: post }
  );
  const { data: relatedArticle, mutate: mutateRelatedArticle } = useSWR(
    `${API_ENDPOINT_ARTICLE}?orderBy=popular&ordering=DESC&limit=${limit}&page=${1}`,
    client.get
  );
  const {
    data: comments,
    ...
    () =>
      postClient?.slug ? `${API_ENDPOINT_COMMENT}/${postClient.slug}` : null,
    client.get,
    { refreshInterval: 1000 * 60, revalidateOnFocus: true }
  );
  const { data: profile } = useSWR<Profile>(
    () => (session ? `${API_ENDPOINT_PROFILE}` : null),
    client.get
  );
  ...
  return (
    <Layout isMobile={isMobile} isUserBottomNavbar={false}>
      <Script id="embedig" src="https://www.instagram.com/embed.js" />
      ...
      {router.isFallback ? (
        ...
        <>
          ...
          >
            {isMobile ? (
              <Mobile post={postClient} />
            ) : (
              <Dekstop post={postClient} />
            )}
                    ...
                    <Disclosure.Panel>
                      <div className="pt-4 pb-24 ">
                        ...
                        <PostCommentAdd
                          ...
                          parrentComment={findCommentById(
                            comments,
                            parrentCommentId
                          )}
                        />
          ...
          <Container className="mt-8 mb-24 md:mt-12">
            {relatedArticle?.length > 0 && (
              <MorePosts
                mutate={() => mutateRelatedArticle()}
                posts={relatedArticle}
                title="Rekomendasi Artikel"
                description="Rekomendasi Artikel untuk anda"
              />
            )}
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  const post = await client.get(
    `${API_ENDPOINT_DETAIL_ARTICLE}/${context.params.slug}`,
    undefined,
