...
export default async function authHeader(tokenParams) {
  ...
  if (typeof window !== 'undefined') { // client side
    if (localStorage.getItem('komika-key') === 'undefined' || localStorage.getItem('komika-key') === 'null') {
...
export async function client(url, {
  ...
  const header = await authHeader(token)
  return fetch(`${bashUrl}${url}`, {
    ...
    .catch(async (err) => {
      console.log('🚀 ~ file: clientRaw.js ~ line 57 ~ err', err.status)
